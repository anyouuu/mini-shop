<template>
  ...
  <div class="user">
        ...
        <el-table size="small" :data="tableData.data" border style="width: 100%"
          ...
          <el-table-column fixed="right" label="操作" width="120">
            <template slot-scope="scope">
              ...
              <el-button @click="cancelClick(scope.row)" v-if="scope.row.deliver_source!=10" type="text" size="small" v-auth="'/takeout/deliver/cancel'">取消配送
              </el-button>
              <el-button @click="verifyClick(scope.row)" v-if="scope.row.deliver_source==10&&scope.row.status==10" type="text" size="small" v-auth="'/takeout/deliver/verify'">确认送达
              </el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <Cancel v-if="open_edit" :open_edit="open_edit" :deliver_id="deliver_id" :order_no="order_no" @closeDialog="closeDialogFunc($event, 'edit')">
    </Cancel>
  </div>
</template>
