		...
		<view class="d-b-c time_picker">
			<view class="" style="width: 40%;">
				<scroll-view style="height: 600rpx;" scroll-y="true">
					<view class="" v-for="(item,index) in hours"
						:class="item.start!=pickhours.start?'hours':'hours-active'" :key='index'
						v-if="myhours<=item.end" @click="pickH(item)">
			...
			<view class="flex-1">
				<scroll-view style="height: 600rpx;" scroll-y="true">
					<view class="" v-for="(item,index) in minute" :key='index'
						v-if="myhours>=pickhours.start&&myminute<=item&&myhours!=inittime(pickhours.end)"
						@click="picktime(pickhours.start+':'+item)">
						{{pickhours.start+":"+item}}
					</view>
					<view class="" v-for="(item,index) in minute" :key='index'
						v-if="myhours<=pickhours.start&&myhours!=inittime(pickhours.end)"
						@click="picktime(pickhours.start+':'+item)">
						{{pickhours.start+":"+item}}
					</view>
					<view class="" v-for="(item,index) in minute" :key='index' v-if="myhours!=inittime(pickhours.end)"
						@click="picktime(inittime(pickhours.end) +':'+item)">
						{{inittime(pickhours.end) +":"+item}}
					</view>
					<view class="" v-for="(item,index) in minute" :key='index'
						v-if="myminute<=item&&myhours==inittime(pickhours.end)"
						@click="picktime(inittime(pickhours.end) +':'+item)">
						{{inittime(pickhours.end)+":"+item}}
					</view>
				</scroll-view>
