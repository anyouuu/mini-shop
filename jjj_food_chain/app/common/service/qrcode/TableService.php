...
class TableService extends Base
{
    ...
    public function getImage()
    {
        ...
        if($this->source == 'wx'){
            // 下载小程序码
            $this->saveQrcodeToDir($table['app_id'], 'pages/product/list/store', $savePath, $this->id,$table['shop_supplier_id']);
        }else if($this->source == 'mp' || $this->source == 'h5'){
            $this->saveMpQrcodeToDir('h5/pages/product/list/store',$savePath, $this->id, $table['app_id'],$table['shop_supplier_id']);
        }
