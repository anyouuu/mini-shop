...
class User extends UserModel
{
    ...
    {
        if ($shop_user_id > 0) {
            return $this->where($where)->where('user_type', '=', 0)->where('shop_user_id', '<>', $shop_user_id)->count();
        }
        return $this->where($where)->where('user_type', '=', 0)->count();
    }

    public function getSupplierUserName($where, $shop_user_id = 0)
    {
        if ($shop_user_id > 0) {
            return $this->where($where)->where('user_type', '=', 1)->where('shop_user_id', '<>', $shop_user_id)->count();
        }
        return $this->where($where)->where('user_type', '=', 1)->count();
    }
