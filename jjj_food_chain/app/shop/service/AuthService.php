...
class AuthService
{
    ...
    private function __construct()
    {
        ...
        // 当前用户信息
        $shop_user_id = !empty($this->store['user']['shop_user_id']) ? $this->store['user']['shop_user_id'] : NULL;

        $this->user = User::detail($shop_user_id);
    }
    ...
    {
        // 超级管理员无需验证
        if (!empty($this->user['is_super']) ?  $this->user['is_super'] : null) {
            return true;
        }
        // 验证当前请求是否在白名单
